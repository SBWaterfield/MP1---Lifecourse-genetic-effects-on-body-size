...
for (yVal in DXAlist){
  ...
  TotFat$Height <- scale(TotFat$Height, scale = F)
  ...
  if (yVal[7] == 'Total Fat Mass'){
    write.table(femresults, file = "TotFatComSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Trunk Fat Mass'){
    write.table(femresults, file = "TrunkFatComSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Total Lean Mass'){
    write.table(femresults, file = "LeanComSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Total Bone Mass'){
    write.table(femresults, file = "BoneComSEX.tsv", row.names=FALSE, sep="\t")
  }
  #------------------------------------------------------------------------------#
  ...
  if (yVal[7] == 'Total Fat Mass'){
    tempminfat <-MinVal
    tempmaxfat <-MaxVal
    tempmin <- c(tempmin, tempminfat)
    tempmax <- c(tempmax, tempmaxfat)
    ...
    tempminlean <-MinVal
    tempmaxlean <-MaxVal
    tempmin <- c(tempmin, tempminlean)
    tempmax <- c(tempmax, tempmaxlean)
  ...
  if (yVal[7] == 'Total Fat Mass'){
    print(ggplot(lincommales, aes(x=age, y=Estimate, group=GRS, color=GRS)) + 
      ...
      ggtitle('Total fat mass difference') +ylab('Percentage difference (%)')+        
        xlab('Age (Years)')+
      geom_hline(yintercept = 0) + ylim(Overallmin, Overallmax) + My_Theme)
    Sys.sleep(0)
    write.table(data_reshapefemale, file = "Results/TotFatDiffComSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Total Fat Mass'){
  ...
  if (yVal[7] == 'Trunk Fat Mass'){
    print(ggplot(lincommales, aes(x=age, y=Estimate, group=GRS, color=GRS)) + 
            geom_errorbar(aes(ymin=`2.5 %`, ymax=`97.5 %`), 
                          position=position_dodge(width=0.3),width=0.2) +
            geom_point(position=position_dodge(width=0.3)) + 
      ggtitle('Trunk fat mass difference') +ylab('Percentage difference (%)')+
        xlab('Age (Years)')+
      geom_hline(yintercept = 0) + ylim(MinVal, MaxVal) +
        theme(plot.title = element_text(size = 15), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
              panel.background = element_blank(), axis.line = element_line(colour = "black")))
    Sys.sleep(0)
    write.table(data_reshapefemale, file = "Results/TrunkFatDiffComSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Trunk Fat Mass'){
    print(ggplot(lincomfemales, aes(x=age, y=Estimate, group=GRS, color=GRS)) + 
            geom_errorbar(aes(ymin=`2.5 %`, ymax=`97.5 %`), 
                          position=position_dodge(width=0.3),width=0.2) +
            geom_point(position=position_dodge(width=0.3)) + 
      ggtitle('Trunk fat mass difference') +ylab('Percentage difference (%)')+xlab('Age (Years)')+
      geom_hline(yintercept = 0) + ylim(MinVal, MaxVal) +
        theme(plot.title = element_text(size = 15), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
              panel.background = element_blank(), axis.line = element_line(colour = "black")))
    Sys.sleep(0)
  ...
  if (yVal[7] == 'Total Lean Mass'){
    print(ggplot(lincommales, aes(x=age, y=Estimate, group=GRS, color=GRS)) + 
            geom_errorbar(aes(ymin=`2.5 %`, ymax=`97.5 %`), 
                          position=position_dodge(width=0.3),width=0.2) +
            geom_point(position=position_dodge(width=0.3)) + 
      ggtitle('Total lean mass difference') +ylab('Percentage difference (%)')+xlab('Age (Years)')+
      geom_hline(yintercept = 0) + ylim(Overallmin, Overallmax) + My_Theme)
    Sys.sleep(0)
    write.table(data_reshapefemale, file = "Results/LeanFatDiffComSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Total Lean Mass'){
    print(ggplot(lincomfemales, aes(x=age, y=Estimate, group=GRS, color=GRS)) + 
            geom_errorbar(aes(ymin=`2.5 %`, ymax=`97.5 %`), 
  ...
  if (yVal[7] == 'Total Bone Mass'){
    print(ggplot(lincommales, aes(x=age, y=Estimate, group=GRS, color=GRS)) + 
            geom_errorbar(aes(ymin=`2.5 %`, ymax=`97.5 %`), 
                          position=position_dodge(width=0.3),width=0.2) +
            geom_point(position=position_dodge(width=0.3)) + 
      ggtitle('Total bone mass difference') +ylab('Percentage difference (%)')+xlab('Age (Years)')+
      geom_hline(yintercept = 0) + ylim(MinVal, MaxVal) +
        theme(plot.title = element_text(size = 15), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
              panel.background = element_blank(), axis.line = element_line(colour = "black")))
    Sys.sleep(0)
...
# Rate of change per spline
  ...
  if (yVal[7] == 'Total Fat Mass'){
    write.table(femmean, file = "TotFatrateComSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Trunk Fat Mass'){
    write.table(femmean, file = "TrunkFatrateComSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Total Lean Mass'){
    write.table(femmean, file = "LeanrateComSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Total Bone Mass'){
    write.table(femmean, file = "BonerateComSEX.tsv", row.names=FALSE, sep="\t")
  }
