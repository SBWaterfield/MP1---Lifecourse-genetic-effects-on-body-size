...
for (yVal in DXAlist){
  ...
  if (yVal[7] == 'Total Fat Mass'){
    write.table(femresults, file = "FemTotFatNOSEX.tsv", row.names=FALSE, sep="\t")
    write.table(malresults, file = "MalTotFatNOSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Trunk Fat Mass'){
    write.table(femresults, file = "FemTrunkFatNOSEX.tsv", row.names=FALSE, sep="\t")
    write.table(malresults, file = "MalTrunkFatNOSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Total Lean Mass'){
    write.table(femresults, file = "FemLeanNOSEX.tsv", row.names=FALSE, sep="\t")
    write.table(malresults, file = "MalLeanNOSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Total Bone Mass'){
    write.table(femresults, file = "FemBoneNOSEX.tsv", row.names=FALSE, sep="\t")
    write.table(malresults, file = "MalBoneNOSEX.tsv", row.names=FALSE, sep="\t")
  }
  #------------------------------------------------------------------------------#
  ...
  if (yVal[7] == 'Total Fat Mass'){
    ...
    Sys.sleep(0)
    write.table(data_reshapemale, file = "Results/MalTotFatDiffNOSEX.tsv", row.names=FALSE, sep="\t")
    write.table(data_reshapefemale, file = "Results/FemTotFatDiffNOSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Total Fat Mass'){
    ...
    print(ggplot(lincommales, aes(x=age, y=Estimate, group=GRS, color=GRS)) + 
      ...
      geom_hline(yintercept = 0) + ylim(MinVal, MaxVal) +
        theme(plot.title = element_text(size = 15), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
              panel.background = element_blank(), axis.line = element_line(colour = "black")))
    Sys.sleep(0)
    write.table(data_reshapemale, file = "Results/MalTrunkFatDiffNOSEX.tsv", row.names=FALSE, sep="\t")
    write.table(data_reshapefemale, file = "Results/FemTrunkFatDiffNOSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Trunk Fat Mass'){
    print(ggplot(lincomfemales, aes(x=age, y=Estimate, group=GRS, color=GRS)) + 
            geom_errorbar(aes(ymin=`2.5 %`, ymax=`97.5 %`), 
    ...
    print(ggplot(lincommales, aes(x=age, y=Estimate, group=GRS, color=GRS)) + 
      ...
      geom_hline(yintercept = 0) + ylim(MinVal, MaxVal) +
        theme(plot.title = element_text(size = 15), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
              panel.background = element_blank(), axis.line = element_line(colour = "black")))
    Sys.sleep(0)
    write.table(data_reshapemale, file = "Results/MalLeanFatDiffNOSEX.tsv", row.names=FALSE, sep="\t")
    write.table(data_reshapefemale, file = "Results/FemLeanFatDiffNOSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Total Lean Mass'){
    print(ggplot(lincomfemales, aes(x=age, y=Estimate, group=GRS, color=GRS)) + 
            geom_errorbar(aes(ymin=`2.5 %`, ymax=`97.5 %`), 
...
# Rate of change per spline
  ...
  if (yVal[7] == 'Total Fat Mass'){
    write.table(femmean, file = "FemTotFatrateNOSEX.tsv", row.names=FALSE, sep="\t")
    write.table(malmean, file = "MalTotFatrateNOSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Trunk Fat Mass'){
    write.table(femmean, file = "FemTrunkFatrateNOSEX.tsv", row.names=FALSE, sep="\t")
    write.table(malmean, file = "MalTrunkFatrateNOSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Total Lean Mass'){
    write.table(femmean, file = "FemLeanrateNOSEX.tsv", row.names=FALSE, sep="\t")
    write.table(malmean, file = "MalLeanrateNOSEX.tsv", row.names=FALSE, sep="\t")
  }
  if (yVal[7] == 'Total Bone Mass'){
    write.table(femmean, file = "FemBonerateNOSEX.tsv", row.names=FALSE, sep="\t")
    write.table(malmean, file = "MalBonerateNOSEX.tsv", row.names=FALSE, sep="\t")
  }
