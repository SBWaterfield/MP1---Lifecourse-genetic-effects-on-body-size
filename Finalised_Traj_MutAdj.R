...
for (yVal in DXAlist){
  ...
  if (yVal[7] == 'Total Lean Mass'){
    plottitle_M <- sprintf('Male %s ', yVal[7])
    print(ggplot(data=Males, aes(x=Age, y=Traj, group=GRS, color = GRS)) +
          geom_line(size=2) + ylab('Lean Mass (Kg)') + xlab('Age (Years)') + ggtitle(plottitle_M) +
          ylim(MinVal, MaxVal) + xlim(9,25) + My_Theme)
  }
  
  if (yVal[7] == 'Total Lean Mass'){
    plottitle_F <- sprintf('Female %s ', yVal[7])
    print(ggplot(data=Females, aes(x=Age, y=Traj, group=GRS, color = GRS)) +
            geom_line(size=2) + ylab('Lean Mass (Kg)') + xlab('Age (Years)') + ggtitle(plottitle_F) +
            ylim(MinVal, MaxVal) + xlim(9,25) + My_Theme)
  }
  ...
  plottitle_M <- sprintf('Male %s ', yVal[7])
  print(ggplot(data=Males, aes(x=Age, y=Traj, group=GRS, color = GRS)) +
    geom_line(size=2) + ylab('Fat Mass (Kg)') + xlab('Age (Years)') + ggtitle(plottitle_M) +
      xlim(9,25) + ylim(MinVal, MaxVal) + My_Theme)
  }
  
  if (yVal[7] == 'Total Fat Mass' | yVal[7] == 'Trunk Fat Mass'){
  plottitle_F <- sprintf('Female %s ', yVal[7])
  print(ggplot(data=Females, aes(x=Age, y=Traj, group=GRS, color = GRS)) +
    geom_line(size=2) + ylab('Fat Mass (Kg)') + xlab('Age (Years)') + ggtitle(plottitle_F) +
      xlim(9,25) + ylim(MinVal, MaxVal) + My_Theme)
  }
